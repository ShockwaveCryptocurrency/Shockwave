...
#if defined(NDEBUG)
# error "Shockwave cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Shockwave: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Shockwave Signed Message:\n";
...
{
    // Shockwave: IsDust() detection disabled, allows any valid dust to be relayed.
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    if (fCheckInputs)
    {
        ...
        int1024_t txMinFee = tx.GetMinFee(1000, true, GMF_RELAY);
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Shockwave: 840k blocks in ~3 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 100; // Shockwave: 3.5 days
static const int64 nTargetSpacing = 100; // Shockwave: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shockwave: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shockwave: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Wikipedia 2017-10-20 The first observation of a neutron star merger in gravitational waves – as well as gamma rays, visible light, and other forms of electromagnetic radiation – is announced.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184b10fa689ad5023690c80fba49c8f13f8b45b8c857aaafffff4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508555054;
        ...
        assert(block.hashMerkleRoot == uint256("0x8a48b640ca7d77798ea2df94ff37c53358cfb52d4d55fb1eb29101d43bed853f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Shockwave: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShockwaveMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShockwaveMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShockwaveMiner(CWallet *pwallet)
{
    printf("ShockwaveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shockwave-miner");
    ...
    try { loop {
        ...
        printf("Running ShockwaveMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShockwaveMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShockwaveMiner, pwallet));
}
